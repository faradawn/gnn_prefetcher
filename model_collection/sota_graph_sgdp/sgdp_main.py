...
from sgdp_model import *
from tqdm import tqdm
from collections import Counter
from sgdp_cache import *
...
def trace2input(dicts, trace, window_size=32):
    ...
    for i in tqdm(range(len(trace)-window_size-1)):
        ...
        for j in range(i, i+window_size+1):
            ...
            op_id = operation_id_map.get(diff) 
            
            if op_id is not None and op_id in keys:
                input_single.append(bo_map[op_id] + 1)###
            else:
...
def dataset2input(dataset, window_size=32, method='top', top_num=1000):
    if method == 'top':
        ...
        lba_trace = 'dataset/MSR-Cambridge/' + dataset + '.csv.gz'
        print('\n', lba_trace, '\n')
        names = ['ByteOffset', 'TimeStamp']
        df = pd.read_csv(lba_trace, compression='gzip', engine='python', skiprows=1, header=None, 
...
def score_compute(all_preds,all_targets,save_name):
    ...
    for i in range(1,len(all_preds[0])):
        hits = [1 if t in p[:i] else 0 for t, p in zip(all_targets, all_preds)]
        pre_list.append(np.mean(hits))
        ranks = []
        for t, p in zip(all_targets, all_preds):
            # Find where the target is in the top-i
            idx = np.where(p[:i] == t)[0]
            if len(idx) > 0:
                ranks.append(1.0 / (idx[0] + 1))
            else:
                ranks.append(0.0)
        mmr_list.append(np.mean(ranks))
...
def main():
    dataset_col = ['hm_1']#'ftds_0802_1021_trace','ftds_0805_17_trace',
    deviceID = 1
