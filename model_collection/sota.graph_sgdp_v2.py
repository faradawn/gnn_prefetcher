...
import sota_graph_sgdp.main as sgdp_wrapper
...
MAX_HIST_IO_BUFFER = 32     # This must be 32; same as SGDP input features size
...
def sgdp_prefetcher(io_queue):
    # Will trigger SGDP inference
    last_lba = io_queue[-1][0]
    hitorical_deltas = get_historical_deltas(io_queue)
    # print("last_lba", last_lba)
    # print("hitorical_deltas", hitorical_deltas)
    lba_to_prefetch = sgdp_wrapper.predict_next_lba(last_lba, hitorical_deltas)
    # print("lba_to_prefetch", lba_to_prefetch)

    offset_to_prefetch, delta = get_lba_to_prefetch(io_queue)
    # print("count = " + str(count) + " delta = " + str(delta) + " io_queue = " + str(io_queue))
    if offset_to_prefetch != None: 
        if io_queue == None:
...
def check_cached_streams(lba_key, time_gap_ms):
    ...
    if lba_key in CACHED_STREAMS:
        ...
        # SGDP will cache the missing LBA
        add_to_cache(lba_key, [], SIZE_PER_KEY)
        return 0
...
def start_processing(memory_size, cache_size, input_path, df): 
...
# Simulate IO submission 
    ...
    for idx, io in df.iloc[1:].iterrows():
        ...
        io_queue.append([curr_offset_kb, delta]) # push

        if len(io_queue) == MAX_HIST_IO_BUFFER:
            #  SGDP prefetching
            # print("len(io_queue)", len(io_queue))
            sgdp_prefetcher(io_queue)
            # exit(0)
...
if __name__ == '__main__':
    ...
    for trace_path in trace_files:
        ...
        model, dicts = sgdp_wrapper.train_model(trace_path)
        sgdp_wrapper.set_model_globaly(model, dicts)
        # print("Finish training the model")
        print("Start testing and simulating the online prefetching")

        df = read_file(trace_path)
        # use the last 50% for testing
        df = df.tail(int(len(df) * 0.5) - 1).reset_index(drop=True)
        start_processing(memory_size, cache_size, trace_path, df)

# How to run:
# ./analyze_read_pattern.py -file ../dataset/iotrace/alibaba.cut.per_10k.rw_80_20.723.trace
