...
# 1 - find trend
def find_trend(queue):
    H_size = len(queue)
    w = 4
    maj = None
    while w <= H_size:
        maj = boyer_moore(queue[len(queue) - w:])
        w *= 2
        if maj is not None or w > H_size:
            return maj
    return maj

# 2 - get prefetch size 
# TODO: adjust size based on prefetch effectiveness (cache hit) 
def get_prefetch_size():
    return SIZE_PER_KEY

# 3 - do prefetch
def do_prefetch(queue):
    prefetch_size = get_prefetch_size()
    if prefetch_size > 0:
        maj = find_trend(queue)
        if maj is not None:
            add_to_cache(queue[-1], prefetch_size)
            return maj
        else: # prefetch size is none-zero, so prefetch around the last majority
            return None
    return None
...
def check_cached_streams(lba_key, queue, time_gap_ms):
    ...
    # TODO: HERE, record cache hits since last prefetch
    if lba_key in CACHED_STREAMS:
        ...
        # Leap v1, find the majority in the queue (Faradawn)
        page_to_fetch = do_prefetch(queue)
...
# Simulate IO submission 
    ...
    for idx, io in df.iterrows():
        ...
        while (total_io_size > 0) :
            ...
            if len(io_queue) == MAX_QUEUE_SIZE:
                io_queue.pop(0) # pop
        ...
        assert(len(io_queue) <= MAX_QUEUE_SIZE)
...
# How to run:
# cd /home/daniar/flashnet/model_collection/5_block_prefetching/simulate_sota_prefetcher
# ./sota.leap_v0.py -chunk 256kb -file ../dataset/iotrace/alibaba.cut.per_10k.rw_80_20.723/read_io.trace
